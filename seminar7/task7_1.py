...
def group_rename(count_nums: int, extension_old: str, new_extension: str, diapazon: list[int], new_name=''):
    count = 0
    for file in os.listdir():
        if file.endswith(extension_old):
            count += 1
            Path(file).rename(f"{file.split('.')[0][diapazon[0]:diapazon[1]]}"
                              f"{new_name}{count:0>{count_nums}}.{new_extension}")
            
group_rename(4, 'txt', 'csv', [3, 6], "new")